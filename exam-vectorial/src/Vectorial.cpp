          ...
          for (unsigned int f = 0; f < cell->n_faces(); ++f)
            {
              if (cell->face(f)->at_boundary() &&
                  cell->face(f)->boundary_id() == 0)
                {
                  ...
                  for (unsigned int q = 0; quadrature_boundary->size(); ++q)
                    {
                      for (unsigned int i = 0; i < dofs_per_cell; ++i)
                        {
                          cell_rhs(i) +=
...
double
Vectorial::compute_error(const VectorTools::NormType &norm_type) const
{
